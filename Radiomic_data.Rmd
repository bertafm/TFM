...
load( "colData_def.rda")
load("colData_trans.rda")
...
sum_table %>% flextable::flextable() %>%            # convert to image
  ...
  flextable::save_as_docx(path = "sum_table.docx") # save as docx file
```
...
pdf(paste0(title, "/features_boxplot.pdf"), width = 20, height = 30)
...
![Boxplot feature variables](`r paste0(title, "/features_boxplot.pdf")`)
...
results_p = ConsensusClusterPlus(
    ...
    title = paste0(title, "/CCPResults/Pearson1/"),
...
results_s = ConsensusClusterPlus(
    ...
    pItem = 0.8, # 80% item (feature) resampling
    pFeature = 0.8, # 80% feature resampling
    title = paste0(title, "/CCPResults/Spearman1/"),
    clusterAlg = "hc", # agglomerative hierarchical clustering algorithm
...
p1 <- ggdraw() + draw_image(paste0(title, "/CCPResults/Pearson1/consensus022.png"), scale = 0.9)
p2 <- ggdraw() + draw_image(paste0(title, "/CCPResults/Spearman1/consensus022.png"), scale = 0.9)
...
pdf(paste0(title, "/dendogram_class_p.pdf"), width = 25, height = 10)
plot(wardsmethod_p)
...
write.xlsx(features_clust, "features_clust.xls")
```
...
icl_p = calcICL(results_p, title=title, plot="png")
...
save(assay_features, file="assay_features_clust.rda")
clustering_coldata <- colData
save(clustering_coldata, file = "clustering_coldata.rda")
...
### 5.2 Descriptive tables by Cluster and GOLDCD: 
 ...
 We can compute a graphical cluster-distribution comparision using boxplot (Figure 5) and estimate the relationship between GOLDCD and different clusters performing an ANOVA analysis (just to give an idea about the behaviour of the variables):  
...
save(pca1_clusters, pca_eigen,
     pca_contrib, pca_plot, file = "pca1_results.rda")
```
...
Delete EMPHET analysis, it does not have sense since emphysema is diagnosed through CT scans. 
Categorize `EMPHET` variable combining categories in order to reduce the number of levels. The idea is to combine levels 0.5, 1.5, 2.5, 3.5, 4.5 and <5% into one unique level: `Trivial (<%5)`:
...
write.xlsx(results_anova_pca, "pca1.xlsx")
```
...
#Dharma emph
simulationOutput2 <- simulateResiduals(fittedModel = models_glmer[[2]], plot = F)
jpeg(file="dharma_emph.jpeg")
...
write.xlsx(results_anova_pca, "pca1_vif.xlsx")
...
```{r}
results_anova_pca
write.xlsx(results_anova_pca, file = "pca_results6_vif.xlsx")
...
write.xlsx(summ, file = "effect_sixe.xlsx")
