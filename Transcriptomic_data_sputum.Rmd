...
pdf("volcano_fevvcpd.pdf", height = 6, width = 5)
(p_fevvcpd <- volcanoplot(fit_sva_fevcpd,coef=2,
            highlight=nrow(toptab_fevcpd[toptab_fevcpd$adj.P.Val<0.05,]),
            names=fit_sva_fevcpd$genes$SYMBOL, main="Change in FEVVCPD"))
dev.off()
...
pdf("volcano_fvcpspc.pdf", height = 6, width = 5)
p_fvcpspc <- volcanoplot(fit_sva_fvcpspc,coef=2,
            highlight=nrow(toptab_fvcpspc[toptab_fvcpspc$adj.P.Val<0.05,]),
...
#We performed the same analysis but usinig DESeq2. Results were more or less the same, but enrichment was quite better with voom results. Thus, we will keep working with voom results
...
### Mediation analysis (Radiomic features versus differential expressed genes): 
Outcome will be cluster and covariates will be the DE (to see which genes are related to which radiomic features):
...
clust1 <- assay_features[assay_features$cluster == 1,]
str(clust1)
head(clust1)
```
...
#geneUniverse_v <- featureData(sputum_eset)[which(featureData(sputum_eset)$PROBEID  %in% 
