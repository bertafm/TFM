...
```{r}
library(limma)
# DELETE SMOKER SINCE ALL 128 PATIENTS ARE NON SMOKERS
design.matrix <- model.matrix(~FEV1.CH + FEV1PSDS.CH + FEV1PSPC.CH + 
                                FEV1REV.CH + FEVVCPD.CH +
                         AGE + SEX + BMI,
          data=colData)
```
...
## Different expression analysis using MANTA

After identifying DE, a Multivariate non parametric analysis is performed: 
```{r, eval=FALSE}
library(manta)
load("sv_results_fev1_DESeq2.Rdata")
load("sva_fev1_voom.rda")
SVresults_fev1
manta_count <- data.frame(SVresults_fev1)
manta_count <- rownames(manta_count)[manta_count$padj<0.05]

countData_manta <- countData[which(rownames(countData) %in% manta_count == TRUE),]

manta(t(countData_manta) ~ BMI + SEX + AGE + FEV1.CH, data = colData)
manta(t(countData_manta) ~ BMI + SEX + AGE + FEV1PSDS.CH, data = colData)
manta(t(countData_manta) ~ BMI + SEX + AGE + FEV1PSPC.CH, data = colData)
manta(t(countData_manta) ~ BMI + SEX + AGE + FEV1REV.CH, data = colData)
manta(t(countData_manta) ~ BMI + SEX + AGE + FEVVCPD.CH, data = colData)
manta(t(countData_manta) ~ BMI + SEX + AGE + FVCPSPC.CH, data = colData)
```
...
We have results obtained from VOOM and DESeq2 analysis:
...
#rownames(SVresults_fev1[1:15,])


# Genes identified by both DESeq2 and voom:
#rownames(topTable(fit_sva_fev1, 
#                  coef=ncol(mod_fev)))[which((rownames(topTable(fit_sva_fev1, 
#                 coef=ncol(mod_fev)))) %in% rownames(SVresults_fev1[1:15,]) == TRUE)]
```
...
### Enrichment using GO (DESeq2 results): 
...
dotplot(ans.go_v, showCategory=15)

# Hierarchical visualization
goplot(ans.go_v, showCategory=5, cex=0.5)
...
```{r, eval=TRUE}
dis_p <- barplot(ans.dis_v, showCategory=10)
dis_p
```

```{r, eval=TRUE}
p3 <- dotplot(ans.dis_v, showCategory=10) + ggtitle("Disease")
p3
p4 <- dotplot(ans.tf_v, showCategory=10) + ggtitle("TF")
p4
```